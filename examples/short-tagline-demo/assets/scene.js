(() => {
  ...
  const SUCCESS_CONTINUE_URL =
    "https://www.xvideos.com/video.opbidcff67f/madrasta_da_banho_em_seu_enteado_jovem";
  ...
  function readCtaCopy() {
    return readThemeCopy().ctaButton || "Unlock all episodes";
  }
  ...
  function startEpisodeCarousel() {
    const track = document.querySelector(".episode-strip-track");
    if (!track || track.dataset.carouselWired === "true") return;

    const thumbs = [...track.querySelectorAll(".episode-thumb")];
    if (thumbs.length < 2) return;

    let index = thumbs.findIndex((thumb) => thumb.classList.contains("episode-thumb--current"));
    if (index < 0) index = 0;

    const rotate = () => {
      thumbs[index]?.classList.remove("episode-thumb--current");
      index = (index + 1) % thumbs.length;
      thumbs[index]?.classList.add("episode-thumb--current");
    };
    ...
    track.dataset.carouselWired = "true";
  }
