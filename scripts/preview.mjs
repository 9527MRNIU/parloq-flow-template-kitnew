...
:root{font-family:system-ui,sans-serif;color:#172033;background:#eef1f5}*{box-sizing:border-box}body{margin:0}.toolbar{position:sticky;top:0;z-index:2;display:flex;flex-wrap:wrap;align-items:center;gap:.75rem;padding:.75rem 1rem;background:#fff;border-bottom:1px solid #d8dee9}.toolbar label{display:flex;align-items:center;gap:.4rem;font-size:.82rem;font-weight:700}.toolbar select,.toolbar button{min-height:2.25rem;border:1px solid #c6cedb;border-radius:.5rem;background:#fff;padding:.35rem .65rem}.toolbar button[data-active=true]{background:#172033;color:#fff}.stage{min-height:calc(100vh - 4rem);display:grid;place-items:start center;padding:1.5rem;overflow:auto}.frame-shell{overflow:hidden;background:#fff;border:1px solid #cfd6e2;border-radius:1rem;box-shadow:0 1rem 3rem #1720331f;transition:width .2s,height .2s}.frame-shell iframe{display:block;width:100%;height:100%;border:0}
</style></head><body>
...
createServer(async (request, response) => {
  ...
  if (url.pathname === "/theme") {
    ...
    const localizedCopy = JSON.parse(await readFile(resolve(themeRoot, `locales/${locale}.json`), "utf8"));
    const html = await readFile(resolve(themeRoot, "index.html"), "utf8");
  ...
  console.log(`template preview: http://127.0.0.1:${port}`);
});
