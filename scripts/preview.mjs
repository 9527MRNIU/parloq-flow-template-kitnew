...
<script>
(()=>{const selected=${stateJson},locale=${localeJson},themeCopy=${copyJson};document.documentElement.lang=locale;document.documentElement.dir=['ar','fa','ur'].includes(locale)?'rtl':'ltr';document.addEventListener('DOMContentLoaded',()=>document.querySelectorAll('[data-copy]').forEach(node=>{const key=node.getAttribute('data-copy');if(key&&themeCopy[key])node.textContent=themeCopy[key]}));let previewPairingIssue=0;const previewCodes=['AAAAAAAA'];const nextPreviewPairing=()=>{previewPairingIssue+=1;return{pairingCode:previewCodes[(previewPairingIssue-1)%previewCodes.length],attemptId:'preview-attempt-'+previewPairingIssue,pairingStatus:'code_issued',expiresAt:new Date(Date.now()+180000).toISOString()}};
