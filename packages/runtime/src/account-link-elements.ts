...
const dropdownStyle = `
  ...
  .menu-search{margin-bottom:.2rem}.menu-search-input{width:100%;min-height:2.35rem;padding:.45rem .65rem;border:1px solid var(--account-link-field-border,#cbd5e1);border-radius:.55rem;background:var(--account-link-field-bg,transparent);color:inherit}.menu-search-input:focus{outline:2px solid var(--account-link-focus,#2563eb);outline-offset:1px}.menu-search-input::-webkit-search-cancel-button{cursor:pointer}
  .menu-empty{margin:0;padding:.65rem;text-align:center;opacity:.75;font-size:.875rem}
  .option{display:flex;align-items:center;justify-content:space-between;gap:.75rem;width:100%;min-height:2.5rem;padding:.45rem .65rem;border:0;border-radius:.55rem;background:transparent;color:inherit;text-align:start;cursor:pointer}
  ...
  .flag{display:inline-flex;align-items:center;justify-content:center;width:1.25rem;height:.85rem;flex:0 0 auto;line-height:0}
  .flag svg{display:block;width:100%;height:100%;border-radius:.15rem;box-shadow:0 0 0 1px rgba(15,23,42,.08)}
...
class PhoneNumberField extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const optionRows = this.options.map((item) => `<li><button class="option" part="country-option" type="button" role="option" data-country="${item.country}" aria-selected="${item.country === this.country}"><span class="option-leading">${countryFlagHtml(item.country)}<span>${item.name}</span></span><span class="option-code">${item.calling}</span></button></li>`).join("");
    this.root.innerHTML = `<style>${sharedStyle}${dropdownStyle}
      ...
      .menu-panel{left:0;right:auto;width:min(18rem,calc(100vw - 2rem))}
    </style><div class="field" part="field"><label class="label" part="label" for="phone-input">${copy.phoneLabel}</label><div class="phone" part="phone-shell"><div class="country" part="country-shell"><span class="sr">${copy.countryLabel}</span><button class="trigger" part="country-select" type="button" aria-haspopup="listbox" aria-expanded="false" aria-label="${copy.countryLabel}"><span class="trigger-leading"><span class="flag" part="country-flag" aria-hidden="true">${current ? countryFlagSvg(current.country) : ""}</span><span class="trigger-label" part="country-label">${current?.name || copy.countryLabel}</span></span><span class="chevron" aria-hidden="true">▾</span></button><span class="prefix" part="country-prefix"></span><div class="menu-panel" part="country-menu" hidden><div class="menu-search" part="country-search-shell"><input class="menu-search-input" part="country-search" type="search" enterkeyhint="search" autocomplete="off" placeholder="${copy.countrySearchPlaceholder}" aria-label="${copy.countrySearchPlaceholder}"></div><ul class="menu-list" part="country-menu-list" role="listbox">${optionRows}</ul><p class="menu-empty" part="country-search-empty" hidden>${copy.countrySearchEmpty}</p></div></div><input part="phone-input" id="phone-input" type="tel" inputmode="tel" autocomplete="tel" placeholder="${copy.phonePlaceholder}" aria-describedby="phone-error"></div><p class="error" part="error" id="phone-error" role="alert"></p></div>`;
    this.trigger = this.root.querySelector(".trigger")!;
    this.triggerFlag = this.root.querySelector(".trigger .flag")!;
    this.triggerLabel = this.root.querySelector(".trigger-label")!;
    this.menu = this.root.querySelector(".menu-panel")!;
    this.menuList = this.root.querySelector(".menu-list")!;
    this.searchInput = this.root.querySelector(".menu-search-input")!;
    this.emptyState = this.root.querySelector(".menu-empty")!;
    ...
    this.menuList.querySelectorAll<HTMLButtonElement>(".option").forEach((button) => {
  ...
  private openMenu() {
    ...
    const selected = this.menuList.querySelector<HTMLElement>('[aria-selected="true"]');
  ...
  private applyCountryFilter() {
    ...
    let visible = 0;
    this.menuList.querySelectorAll<HTMLButtonElement>(".option").forEach((button) => {
      const item = this.options.find((option) => option.country === button.dataset.country);
      const row = button.closest("li");
      if (!item || !row) return;
      const haystack = `${item.name} ${item.country} ${item.calling}`.toLowerCase();
      const match = !query || haystack.includes(query) || (digits ? item.calling.includes(digits) : false);
      row.hidden = !match;
      if (match) visible += 1;
    });
  ...
  private syncSelected() {
    ...
    this.triggerFlag.innerHTML = current ? countryFlagSvg(current.country) : "";
    this.menuList.querySelectorAll<HTMLButtonElement>(".option").forEach((button) => {
